...
from load_bigram_data import load_bigram_
...
def main():
    if not os.path.exists('plm'): os.makedirs('plm')
    if not os.path.exists('models_save'): os.makedirs('models_save')
    if not os.path.exists('logs'): os.makedirs('logs')

    config = parse_args()
    config.type_id, config.id_type, config.args_id, config.id_args, \
    config.ty_args, config.ty_args_id, config.args_s_id, config.args_e_id = get_dict(config.data_path)
    ...
    tokenizer = tokenizer_class.from_pretrained(config.model_name_or_path, do_lower_case=config.do_lower_case,
                                                cache_dir=config.cache_dir if config.cache_dir else None)
    # tokenizer_eng = tokenizer_class.from_pretrained('bert-base-cased', do_lower_case=config.do_lower_case,
    #                                                 cache_dir=config.cache_dir if config.cache_dir else None)
    ...
    # sets, datasets, vocabs, embeddings = load_bigram_()
    if config.do_train:
        train_set = Data(task='train', fn=config.data_path + '/cascading_sampled/new_train.json', tokenizer=tokenizer,
                         seq_len=config.seq_length, args_s_id=config.args_s_id, args_e_id=config.args_e_id,
                         type_id=config.type_id)
        train_loader = DataLoader(train_set, batch_size=config.batch_size, shuffle=False, collate_fn=collate_fn_train)
        dev_set = Data(task='eval_with_oracle', fn=config.data_path + '/cascading_sampled/new_dev.json',
                       tokenizer=tokenizer, seq_len=config.seq_length, args_s_id=config.args_s_id,
                       args_e_id=config.args_e_id, type_id=config.type_id)
        ...
        # framework.extra_bigrams(train_loader, dev_loader)
    ...
    if config.do_eval:
        ...
        dev_set = Data(task='eval_with_oracle', fn=config.data_path + '/cascading_sampled/new_dev.json',
                       ...
                       type_id=config.type_id)
        dev_loader = DataLoader(dev_set, batch_size=1, shuffle=False, collate_fn=collate_fn_dev)
        c_ps, c_rs, c_fs, t_ps, t_rs, t_fs, a_ps, a_rs, a_fs = \
            framework.evaluate_with_oracle(config, model, dev_loader, config.device, config.ty_args_id, config.id_type)
    ...
    if config.do_test:
        ...
        dev_set = Data(task='eval_with_oracle', fn=config.data_path + '/cascading_sampled/new_test.json',
                       tokenizer=tokenizer,
                       seq_len=config.seq_length, args_s_id=config.args_s_id, args_e_id=config.args_e_id,
                       type_id=config.type_id)
        dev_loader = DataLoader(dev_set, batch_size=1, shuffle=False, collate_fn=collate_fn_dev)
        ...
        dev_set = Data(task='eval_without_oracle', fn=config.test_path, tokenizer=tokenizer, seq_len=config.seq_length,
                       args_s_id=config.args_s_id, args_e_id=config.args_e_id, type_id=config.type_id)
        ...
        prf_s, pred_records = \
            framework.evaluate_without_oracle(config, model, dev_loader, config.device, config.seq_length,
                                              config.id_type, config.id_args, config.ty_args_id)
